...
public class AutoGeneratedCSharpMigrationsGenerator : CSharpMigrationsGenerator
{
    ...
    private static string WarningDisablers =>
    ...
    )
    {
        ...
        builder.AppendLine();
        builder.AppendLine(WarningDisablers);
        builder.AppendLine();

        // Find where to insert our modifications
        var firstUsing = originalMigration.First(s => s.Contains("using"));
        var usingIndent = new string(firstUsing.TakeWhile(c => c is ' ' or '\t').ToArray());
        ...
        var classIndent = new string(classDeclaration.TakeWhile(c => c is ' ' or '\t').ToArray());
